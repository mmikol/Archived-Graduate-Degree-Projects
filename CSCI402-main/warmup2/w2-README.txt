...
Comments on design decisions: I think your expected outputs in the f1.txt file are incorrect. 
    It specifies a bucket capacity that is smaller than many of the packet's demand for tokens. 
    Yet, your expected statistic for dropped packets is 0. This doesn't make sense since we must 
    drop a packet once we find that it's demand exceeds the token bucket's capacity (which my program does correctly). It won't matter but I must say that the past two specs have been horribly written.
It's as if the professor is adding new things to the spec via the group and editing the website. Moreover, the spec is not organized in a single document and I have to visit several places to get clarification (i.e. spec, slides, FAQ, google groups, zoom lectures, etc). 
